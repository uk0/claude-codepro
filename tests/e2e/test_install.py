...
class TestHooksAbsolutePaths:
    """Test that hooks work from subdirectories with absolute paths."""

    def test_hooks_work_from_subdirectory(self, installed_project):
        """Test that hooks execute correctly from nested subdirectories."""
        # Create nested directory structure
        nested_dir = installed_project / "src" / "components" / "deep" / "nested"
        nested_dir.mkdir(parents=True)

        # Verify settings has absolute paths
        settings_file = installed_project / ".claude" / "settings.local.json"
        settings_data = json.loads(settings_file.read_text())

        # Get hook path (command is "python3 /path/to/hook.py")
        hook_path = None
        for hook_group in settings_data["hooks"]["PostToolUse"]:
            for hook in hook_group.get("hooks", []):
                command = hook.get("command", "")
                if "file_checker_qlty.py" in command:
                    # Extract path from "python3 /path/to/hook.py"
                    parts = command.split()
                    if len(parts) >= 2:
                        hook_path = parts[1]
                    break
        ...
        assert hook_file.is_absolute()

        # Test that hook is executable from nested directory
        ...
        # Hook should run without error (exit code 0 or 2 for success)
